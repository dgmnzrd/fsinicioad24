...
<body>
    <marquee id="marquee" class="blink" behavior="scroll" direction="right", scrollamount="20">
        <h1 id="h1" style="color: white">Editar Medicina</h1>
    </marquee>
    <form id="miFormulario" action="{{ route( 'medicinas.update', $medicina->MEDICINAS_ID ) }}" method="POST">
        @csrf
        <div>
            <label for="Clave">Clave: </label>
            <input type="text" id="clave" name="clave" value="{{ $medicina->CLAVE }}">
        </div>
        <div>
            <label for="Nombre">Nombre: </label>
            <input type="text" id="nombre" name="nombre" value="{{ $medicina->NOMBRE }}">
        </div>
        <div>
        <label for="Precio">Precio: </label>
            <input type="text" id="precio" name="precio" value="{{ $medicina->PRECIO }}">
        </div>
        @method( 'PUT' )
